...
#ifndef CGOGN_MODULE_GRAPH_DEFORMATION_H_
#define CGOGN_MODULE_GRAPH_DEFORMATION_H_
...
class GraphDeformation : public ViewModule
{
	...
	struct Parameters
	{
		Parameters()
			...
			vertex_position_init_(nullptr), vertex_area_(nullptr),
			  ...
			  edge_weight_(nullptr), normal_({0.0, 0.0, 0.0})
		{
		}
...
public:
	GraphDeformation(const App& app)
		: ViewModule(app, "GraphDeformation (" + std::string{mesh_traits<MESH>::name} + ")"), selected_mesh_(nullptr),
...
public:
	...
	void set_displacement_normal(const MESH& m, const Vec3& normal){
		Parameters& p = parameters_[&m];
		p.normal_ = normal;
	}
...
protected:
	void init() override
	{
		...
		connections_.push_back(boost::synapse::connect<typename MeshProvider<MESH>::mesh_added>(
			mesh_provider_, this, &GraphDeformation<MESH>::init_mesh));
	}
...
#endif // CGOGN_MODULE_GRAPH_DEFORMATION_H_
