		...
		mesh_connections_[m].push_back(
			boost::synapse::connect<typename MeshProvider<MESH>::
				...
				m, [this, m](MeshAttribute<Vec3>* attribute) {
					...
					{
						p.vertex_base_size_ =  float32(geometry::mean_edge_length(*m, 
	...
	*/
		...
		const std::string& binding_type)
	{
		...
		if (axis_container_.size())
		{
			for (auto& [name, adt] : axis_container_)
			{
				if (gcdt->local_axis_weights_.count(name) == 0){
		...
		if (cage_container_.size())
		{
			for (auto& [name, cdt] : cage_container_)
			{
				if (gcdt->local_cage_weights_.count(name) == 0){
					modeling::Parameters<MESH>& p_cage = 
								*parameters_[cdt->control_cage_];

					gcdt->init_bind_local_cage(*(cdt->control_cage_), name, 
		...
		gcdt->cage_attribute_update_connection_ =
			boost::synapse::connect<typename MeshProvider<MESH>::
				...
				&global_cage, [&](MeshAttribute<Vec3>* attribute) {
					...
					{
						if ((deformed_tool_ == Cage) && (cage_type_ == "global"))
						{
							...
							if (current_gcdt->local_axis_weights_.size() > 0)
							{
										...
										current_gcdt->local_axis_weights_)
								{
									...
									*axis_parameters_[local_adt->control_axis_];
									...
									local_adt->require_full_binding(); 
							...
							{
								for (auto& [name, mw] : 
										current_gcdt->local_cage_weights_)
								{
									std::shared_ptr<modeling::
									CageDeformationTool<MESH>> local_cdt =
										cage_container_[name];

									modeling::Parameters<MESH>& p_cage =
									*parameters_[local_cdt->control_cage_];

									current_gcdt->deform_local_cage(
										*(local_cdt->control_cage_), name,
										p_cage.vertex_position_, 
										local_cdt->control_cage_vertex_index_);

									local_cdt->require_full_binding(); 
		...
		std::string binding_type)
	{
		...
		cdt->init_bind_object(object, object_vertex_position.get(), 
								object_vertex_index.get());
		...
		if (cage_container_.size())
		{
			for (auto& [name, other_cagedt] : cage_container_)
			{
				if (!(name == p_cage.name_)){ 

					if (cdt->local_cage_data_.count(name) == 0){

						modeling::Parameters<MESH>& p_target_cage = 
								*parameters_[other_cagedt->control_cage_];

						cdt->init_bind_local_cage(*(other_cagedt->control_cage_), name, 
						other_cagedt->control_cage_vertex_position_, other_cagedt->control_cage_vertex_index_); 
				}
				}
		...
		if (global_cage_container_.size() > 0)
		{
			...
			if (gcdt->local_cage_weights_.count(p_cage.name_) == 0){

				gcdt->init_bind_local_cage(*(cdt->control_cage_), p_cage.name_, 
									cdt->control_cage_vertex_position_, cdt->control_cage_vertex_index_);
			} 
		}
		...
		cdt->cage_attribute_update_connection_ =
			boost::synapse::connect<typename MeshProvider<MESH>::
				...
				&local_cage, [&](MeshAttribute<Vec3>* attribute) 
				{
					if (cage_vertex_position.get() == attribute)
					{
						if ((deformed_tool_ == Cage) && (cage_type_ == "local"))
						{
							std::shared_ptr<modeling::
							...
							current_cdt->deform_object(object, 
											object_vertex_position.get(), 
											object_vertex_index.get(), handle_container_, 
												axis_container_, 
												cage_container_);
							...
							if (current_cdt->local_cage_data_.size() > 0)
							{
								for (auto& [name, cage_data] : 
										current_cdt->local_cage_data_)
								{
									std::shared_ptr<modeling::
									CageDeformationTool<MESH>> local_cdt =
										cage_container_[name];
		...
		std::string binding_type)
	{
		...
		cdt->update_bind_object(object, object_vertex_position.get(), 
								object_vertex_index.get());
							...
							/*if (cage_container_.size() > 0)
							{
								for (auto& [name, cdt] : cage_container_)
								{
									...
									if (cage_index == 27)
									{
										...
										const Vec3 handle_displacement = current_hdt->get_deformation();

										const Vec3 local_displacement = 
										cdt->local_frame_*handle_displacement; 
										...
										if (!(modeling::check_triple_projection_in_area(shifted_pos, cdt->control_cage_local_bb_min_, cdt->control_cage_local_bb_max_)))
										{
											...
											*(cdt->control_cage_), 
											cdt->control_cage_vertex_position_.get());
						...
						if (selected_cage_)
						{
							...
							if (cage_name.size() > 0)
							{
								...
								cage_name.begin()).first == prefix.end())
								{
									...
									if (!selected_cdt_->split_deformation_type_.empty()){
										const char* extension = selected_cdt_->split_deformation_type_.c_str();
								...
								if (ImGui::Button("Bind axis"))
								{
									if (selected_adt_->deformation_type_.empty()){
										bind_local_axis(*model_, 
												model_p.vertex_position_, 
												*(selected_adt_->control_axis_),
										selected_adt_->control_axis_vertex_position_, item_current);
...
private:
	...
	void accept_handle_influence_area()
	{
		...
		current_hdt->set_object_influence_area(*model_, model_vertex_index.get(), influence_set_); 

		//current_hdt->set_euclidean_distance(*model_, model_p.vertex_position_); 
	...
	void check_deformation_handle_shared_vertex(const std::string& handle_name)
	{
		...
		for ( const auto &myPair : current_hdt->shared_vertex_ ) {
				...
				activation_map_[vertex_index].handle_translation_[handle_name] = 
															max_local_distance; 

				if (activation_map_[vertex_index].current_max_handle_.second == 0.0)
				{
				...
				} else if (activation_map_[vertex_index].current_max_handle_.first == handle_name)
				{
					...
					if (max_local_distance > activation_map_[vertex_index].current_max_handle_.second){

						std::string old_max_handle_name = activation_map_[vertex_index].current_max_handle_.first; 
						...
						const Vec3 reset_position = current_position - old_hdt->object_influence_area_[vertex_index].max_local_translation; 
				...
				if (update_value){
					...
					if (activation_map_[vertex_index].name_tool_vertex_ != handle_name)
					{
						...
						std::shared_ptr<modeling::HandleDeformationTool<MESH>> target_hdt =
