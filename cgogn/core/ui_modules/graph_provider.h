...
#ifndef CGOGN_MODULE_GRAPH_PROVIDER_H_
#define CGOGN_MODULE_GRAPH_PROVIDER_H_
...
class GraphProvider : public ProviderModule
{
	template <typename T>
	using Attribute = typename mesh_traits<GRAPH>::template Attribute<T>;
	using AttributeGen = typename mesh_traits<GRAPH>::AttributeGen;
...
public:
		  ...
		  bb_min_(0, 0, 0), bb_max_(0, 0, 0)
	{
		...
		// 	n[0] = '\0';
		if constexpr (mesh_traits<GRAPH>::dimension == 1)
			supported_formats_ = &supported_graph_formats_;
		if constexpr (mesh_traits<GRAPH>::dimension == 2)
			supported_formats_ = &supported_surface_formats_;
		if constexpr (mesh_traits<GRAPH>::dimension == 3)
	...
	{
		if constexpr (std::is_default_constructible_v<GRAPH>)
		{
			const auto [it, inserted] = graphs_.emplace(name, std::make_unique<GRAPH>());
			GRAPH* g = it->second.get();
			if (inserted)
			{
	...
	{
		const std::string& g_name = graph_name(g);
		std::string name =
			remove_extension(g_name) + "_" + std::to_string(number_of_graphs()) + "." + extension(g_name);
		GRAPH* result = add_graph(name);
		copy(*result, g);
		...
		boost::synapse::emit<graph_added>(this, result);
		emit_connectivity_changed(*result);
		// TODO: emit attributes changed ?
		return result;
	}

	void register_graph(GRAPH* g, const std::string& name)
	{
		const auto [it, inserted] = graphs_.emplace(name, std::unique_ptr<GRAPH>(g));
		if (inserted)
		{
	...
	{
		clear(g);
		emit_connectivity_changed(g);
		// TODO: emit attributes changed ?
	}

	void copy_graph(GRAPH& dst, const GRAPH& src)
	{
		copy(dst, src);
	...
	void foreach_graph(const FUNC& f)
	{
		...
		for (auto& [name, g] : graphs_)
			f(*g, name);
	}

	inline uint32 number_of_graphs()
	{
		return uint32(graphs_.size());
	}

	std::string graph_name(const GRAPH& g) const
	{
		auto it =
			std::find_if(graphs_.begin(), graphs_.end(), [&](const auto& pair) { return pair.second.get() == &g; });
		if (it != graphs_.end())
			return it->first;
		else
			return "";
	...
	GraphData<GRAPH>& graph_data(const GRAPH& g)
	{
		return graph_data_[&g];
	}
...
private:
	void update_meshes_bb()
	{
		...
		for (auto& [g, gd] : graph_data_)
		{
			for (uint32 i = 0; i < 3; ++i)
			{
				if (gd.bb_min_[i] < bb_min_[i])
					bb_min_[i] = gd.bb_min_[i];
				if (gd.bb_max_[i] > bb_max_[i])
					bb_max_[i] = gd.bb_max_[i];
			}
		}
	}

public:
	void set_mesh_bb_vertex_position(const GRAPH& g, const std::shared_ptr<Attribute<Vec3>>& vertex_position)
	{
		GraphData<GRAPH>& gd = graph_data(g);
		gd.bb_vertex_position_ = vertex_position;
		gd.update_bb();
		update_meshes_bb();
	...
	using graph_added = struct graph_added_ (*)(GRAPH* g);
	template <typename T>
	...
	using cells_set_changed = struct cells_set_changed_ (*)(CellsSet<GRAPH, CELL>* set);

	template <typename T>
	void emit_attribute_changed(const GRAPH& g, Attribute<T>* attribute)
	{
		GraphData<GRAPH>& gd = graph_data(g);
		gd.update_vbo(attribute);
		if (static_cast<AttributeGen*>(gd.bb_vertex_position_.get()) == static_cast<AttributeGen*>(attribute))
		{
			gd.update_bb();
			update_meshes_bb();
			for (View* v : linked_views_)
		...
		boost::synapse::emit<attribute_changed>(&g, attribute);
		boost::synapse::emit<attribute_changed_t<T>>(&g, attribute);
	}

	void emit_connectivity_changed(const GRAPH& g)
	{
		GraphData<GRAPH>& gd = graph_data(g);
		gd.update_nb_cells();
		gd.rebuild_cells_sets();
		gd.set_all_primitives_dirty();

		for (View* v : linked_views_)
			v->request_update();

		boost::synapse::emit<connectivity_changed>(&g);
	}

	template <typename CELL>
	void emit_cells_set_changed(const GRAPH& g, CellsSet<GRAPH, CELL>* set)
	{
		boost::synapse::emit<cells_set_changed<CELL>>(&g, set);
	}
...
protected:
	void main_menu() override
	{
		...
		if (ImGui::BeginMenu(name_.c_str()))
		{
			if (ImGui::MenuItem("Add graph"))
				add_graph(std::string{mesh_traits<GRAPH>::name});
	...
	void popups() override
	{
		...
		if (ImGui::BeginPopupModal("Save", NULL, ImGuiWindowFlags_AlwaysAutoResize))
		{
			...
			if (selected_graph)
			{
				...
				imgui_combo_attribute_graph<Vertex, Vec3>(
					*selected_graph, selected_vertex_position, "Position",
	...
	{
		imgui_graph_selector(this, selected_graph_, "Graph", [&](GRAPH& g) {
			selected_graph_ = &g;
			graph_data(g).outlined_until_ = App::frame_time_ + 1.0;
		});

		if (selected_graph_)
		{
			GraphData<GRAPH>& gd = graph_data(*selected_graph_);

			imgui_combo_attribute_graph<Vertex, Vec3>(*selected_graph_, gd.bb_vertex_position_, "Position",
												[&](const std::shared_ptr<Attribute<Vec3>>& attribute) {
													set_mesh_bb_vertex_position(*selected_graph_, attribute);
												});

			if (ImGui::Button("Clone"))
				clone_graph(*selected_graph_);

			ImGui::Separator();
			ImGui::TextUnformatted("Size");
			ImGui::Separator();

			if (ImGui::BeginTable("GraphSize", 2))
			{
				ImGui::TableSetupColumn("CellType");
				ImGui::TableSetupColumn("Number");
				ImGui::TableHeadersRow();

				for (uint32 i = 0; i < std::tuple_size<typename mesh_traits<GRAPH>::Cells>::value; ++i)
				{
					ImGui::TableNextColumn();
					ImGui::TextUnformatted(mesh_traits<GRAPH>::cell_names[i]);
					ImGui::TableNextColumn();
					ImGui::Text("%d", gd.nb_cells_[i]);
				}
				ImGui::EndTable();
			}
			...
			if (ImGui::BeginTable("GraphAttributes", 2))
			{
				ImGui::TableSetupColumn("CellType");
				ImGui::TableSetupColumn("Name");
				ImGui::TableHeadersRow();

				auto names = gd.attributes_names();
				for (uint32 i = 0; i < std::tuple_size<typename mesh_traits<GRAPH>::Cells>::value; ++i)
				{
					ImGui::TableNextColumn();
					ImGui::TextUnformatted(mesh_traits<GRAPH>::cell_names[i]);
					...
					if (ImGui::ListBoxHeader((std::string("##") + mesh_traits<GRAPH>::cell_names[i]).c_str(),
											 names[i].size()))
					{
						for (auto& n : names[i])
...
private:
	...
	std::unordered_map<const GRAPH*, GraphData<GRAPH>> graph_data_;
...
#endif // CGOGN_MODULE_GRAPH_PROVIDER_H_
