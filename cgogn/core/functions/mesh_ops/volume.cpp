...
CMap2::Volume add_custom_polyhedron(CMap2& m, uint32 size, bool set_indices, uint32 number_side)
{
	CMap1::Face first = add_face(static_cast<CMap1&>(m), number_side, false); // first quad
	Dart current = first.dart;
	for (uint32 i = 1u; i < size; ++i) // Next quads
	{
		CMap1::Face next = add_face(static_cast<CMap1&>(m), number_side, false);
		phi2_sew(m, phi_1(m, current), phi1(m, next.dart));
		current = next.dart;
	}
	phi2_sew(m, phi_1(m, current), phi1(m, first.dart)); // Finish the sides
