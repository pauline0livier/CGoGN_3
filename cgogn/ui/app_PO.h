...
#ifndef CGOGN_UI_APP_PO_H_
#define CGOGN_UI_APP_PO_H_
...
class CGOGN_UI_EXPORT App_PO
{
	friend class Module_PO;
...
public:
	...
	View_PO* add_view();
	inline View_PO* current_view() const
	{
	...
	{
		static_assert(is_func_parameter_same<FUNC, View_PO*>::value,
					  "Wrong parameter type: given function should take a View*");
		for (const auto& v : views_)
			f(v.get());
	}

	Module_PO* module(const std::string& name) const;
	...
	void start_timer(uint32 interval, FUNC stop_cond) const
	{
		...
		std::thread timer([this, interval, stop_cond]() {
			while (true)
			{
				...
				if (stop_cond())
					return;
				boost::synapse::emit<App_PO::timer_tick>(this);
			}
		});
		timer.detach();
	}
...
private:
	...
	std::vector<std::unique_ptr<View_PO>> views_;
	View_PO* current_view_;

	mutable std::vector<Module_PO*> modules_;
