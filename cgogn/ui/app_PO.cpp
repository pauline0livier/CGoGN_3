	  ...
	  interface_scaling_(1.0f), mouse_scroll_speed_(50.0f), show_imgui_(true), show_demo_(false), current_view_(nullptr)
{
	...
	glfwSetWindowSizeCallback(window_, [](GLFWwindow* wi, int width, int height) {
		App_PO* that = static_cast<App_PO*>(glfwGetWindowUserPointer(wi));
...
void App_PO::set_window_size(int32 w, int32 h)
{
	glfwSetWindowSize(window_, w, h);
}

void App_PO::set_window_title(const std::string& name)
{
	window_name_ = name;
	if (window_)
...
View_PO* App_PO::add_view()
{
	if (uint32(views_.size()) < 4)
	{
		glfwMakeContextCurrent(window_);
		views_.push_back(std::make_unique<View_PO>(&inputs_, "view" + std::to_string(uint32(views_.size()))));
...
Module_PO* App_PO::module(const std::string& name) const
{
	auto it =
		std::find_if(modules_.begin(), modules_.end(), [&](Module_PO* m) { return m->name().compare(name) == 0; });
	if (it != modules_.end())
		return *it;
	return nullptr;
}

void App_PO::close_event()
{
...
void App_PO::adapt_views_geometry()
{
...
void App_PO::init_modules()
{
	for (Module_PO* m : modules_)
		m->init();
}
	...
	while (!glfwWindowShouldClose(window_))
	{
		...
		if (show_imgui_)
		{
			...
			if (ImGui::BeginMainMenuBar())
			{
				...
				{
					if (ImGui::ColorEdit3("Background color", background_color_.data(), ImGuiColorEditFlags_NoInputs))
					{
						for (const auto& v : views_)
							v->request_update();
					}
		...
		if (first_render)
			ImGui::DockBuilderDockWindow("Menu", dockIdLeft);
