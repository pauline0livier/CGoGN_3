...
#ifndef CGOGN_MODULE_SURFACE_SELECTION_PO_H_
#define CGOGN_MODULE_SURFACE_SELECTION_PO_H_
...
class SurfaceSelectionPO : public ViewModule
{
...
public:
	SurfaceSelectionPO(const App& app)
		: ViewModule(app, "SurfaceSelectionPO (" + std::string{mesh_traits<MESH>::name} + ")"), selected_mesh_(nullptr)
	{
	}
...
protected:
	void init() override
	{
		...
		connections_.push_back(boost::synapse::connect<typename MeshProvider<MESH>::mesh_added>(
			mesh_provider_, this, &SurfaceSelectionPO<MESH>::init_mesh));
	}
				...
				case WithinSphere: {
					...
					if (!picked.empty())
					{
						...
						if (back_side){
							...
							CellCache<MESH> cache_back_ = geometry::within_sphere(*selected_mesh_, picked[1],
																		p.vertex_base_size_ * p.sphere_scale_factor_,
																		p.vertex_position_.get());
							...
							if (p.selecting_cell_ == VertexSelect){
								if (p.selected_vertices_set_)
								{
									...
									{
										case 0:
											foreach_cell(cache_back_, [&p](Vertex v) -> bool {
												p.selected_vertices_set_->select(v);
												return true;
											});
...
#endif // CGOGN_MODULE_SURFACE_SELECTION_PO_H_
